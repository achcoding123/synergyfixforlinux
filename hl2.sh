...
if [ "$UNAME" == "Darwin" ]; then
   ...
   # SYNERGY: Added synergy/bin
   export DYLD_LIBRARY_PATH="${GAMEROOT}"/bin:"${GAMEROOT}"/synergy/bin:$DYLD_LIBRARY_PATH
elif [ "$UNAME" == "Linux" ]; then
   # prepend our lib path to LD_LIBRARY_PATH
   # SYNERGY: Added synergy/bin
   export LD_LIBRARY_PATH="${GAMEROOT}"/bin:"${GAMEROOT}"/synergy/bin:$LD_LIBRARY_PATH
fi
...
STATUS=42
ARGS="$@ -game synergy" # Synergy addition
while [ $STATUS -eq 42 ]; do
	if [ "${GAME_DEBUGGER}" == "gdb" ] || [ "${GAME_DEBUGGER}" == "cgdb" ]; then
		...
		echo run $ARGS >> "$ARGSFILE" # Synergy edit: use $ARGS
		echo show args >> "$ARGSFILE"
		${GAME_DEBUGGER} "${GAMEROOT}"/${GAMEEXE} -x "$ARGSFILE"
		rm "$ARGSFILE"
	else
		${GAME_DEBUGGER} "${GAMEROOT}"/${GAMEEXE} $ARGS # Synergy edit: use $ARGS
	fi
	STATUS=$?
done
exit $STATUS
